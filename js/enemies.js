...
var _claudeBotId = 100 + Math.floor(Math.random() * 900);
function _genBotName() {
    _claudeBotId += 1 + Math.floor(Math.random() * 7);
    var prefixes = ['Claw Agent', 'Claw Bot', 'C-Agent', 'Agent-C', 'Claw Unit'];
    var p = prefixes[Math.floor(Math.random() * prefixes.length)];
    return p + ' #' + _claudeBotId;
}
...
function _makeNameSprite(text) {
    ...
    var tagText = 'AI';
    var tagW = c.measureText(tagText).width + 24;
    var nameW = c.measureText(text).width + 20;
    var totalW = tagW + nameW + 16;
    var startX = Math.max(0, (512 - totalW) / 2);
    var boxY = 16;
    var boxH = 96;
    var midY = boxY + boxH / 2;

    c.fillStyle = 'rgba(0,0,0,0.75)';
    _roundRect(c, startX, boxY, totalW, boxH, 16);
    c.fill();

    c.fillStyle = '#6366f1';
    _roundRect(c, startX + 10, boxY + 12, tagW, boxH - 24, 10);
    c.fill();

    c.fillStyle = '#ffffff';
    c.font = 'bold ' + fontSize + 'px Arial, sans-serif';
    c.fillText(tagText, startX + 22, midY + 2);

    c.fillStyle = '#e2e8f0';
    c.font = 'bold ' + (fontSize - 4) + 'px Arial, sans-serif';
    c.fillText(text, startX + tagW + 16, midY + 2);
...
function Enemy() {
    ...
    Enemy.prototype.CreateNameTag = function() {
        this.botName = _genBotName();
        this.nameSprite = _makeNameSprite(this.botName);
        this.nameSprite.position.set(0, 0, 3.2);
